package client.scenes;

import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;

import static org.junit.jupiter.api.Assertions.assertNotNull;

public class ClosestQuestionCtrlTest {

    private MyMainCtrl myMainCtrl;
    private ClosestQuestionCtrl closestQuestion;

    @BeforeEach
    public void setUp() {
        myMainCtrl = new MyMainCtrl();
        closestQuestion = new ClosestQuestionCtrl(myMainCtrl);
    }

    @Test
    public void constructor() {
        assertNotNull(closestQuestion);
    }

    public static class MyMainCtrl extends MainCtrl {

        public int wasCalled = 0;

        @Override
        public void register(GameManager listener) {
            wasCalled ++;
        }
    }

}
